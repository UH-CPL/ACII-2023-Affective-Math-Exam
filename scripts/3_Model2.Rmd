...
# Model 2 - Normalized HR_Validated Model with Participant centered Random Effect 
## Normalized HR_Validated Full Model
...
M2.hr.log.fit.fm <- lmer(hr.e4_validated_normalized ~ (1 | ParticipantID) + SAI.Score +
                    Question.Type + q.solv.time+ grade,
                  data = Qlevel_rl, REML=FALSE
                  )

mod_summary<-summary(M2.hr.log.fit.fm) 
...
# Class 
qc.pm<-plot_model(M2.hr.log.fit.fm, 
             type = "pred", 
             terms = "Question.Type",
             dot.size = 6,
             line.size = 2,
...
# Correctness
ac.pm <- plot_model(M2.hr.log.fit.fm,
             type = "pred", 
             terms = "grade",
             dot.size = 6,
             line.size = 2,
...
# Solving Time
qst.pm <- plot_model(M2.hr.log.fit.fm, 
             type = "pred", 
             terms = "q.solv.time",
             dot.size = 6,
             line.size = 2,
...
sai.pm<-plot_model(M2.hr.log.fit.fm, 
             type = "pred", 
             terms = "SAI.Score",
             dot.size = 6,
             line.size = 2,
...
m.plot <- sjPlot::plot_model(M2.hr.log.fit.fm,
...
step_bw <- step(M2.hr.log.fit.fm, direction = "backward")
...
M2.hr.log.fit.fm.bw <- eval(suggested_fm)


```


## Normalized HR_Validated Optimal Model by BE


```{r echo=FALSE, message=FALSE, warning=FALSE}
summary(M2.hr.log.fit.fm.bw)
aic.bw<- AIC(M2.hr.log.fit.fm.bw)

#aic.bw
cat("\nOptimal Model of BE AIC:" ,aic.bw , "\n\n")

#aic.full
cat("Full Model AIC:" ,AIC(M2.hr.log.fit.fm.bw))
```
...
qc.pm<-plot_model(M2.hr.log.fit.fm.bw, 
             type = "pred", 
             terms = "Question.Type",
             dot.size = 6,
             line.size = 2,
...
```{r echo=FALSE}
hr.pred.final <- ggarrange(qc.pm,ncol = 1, nrow=1)
final_plot =  cowplot::plot_grid(hr.pred.final,mylegend,nrow=2, rel_heights = c(.9,.1))
final_plot

filename <- "HR_FinalModel_Pred_N20.pdf"
plot_dir = "../plots/"
...
qc.pm.re<-plot_model(M2.hr.log.fit.fm.bw, 
             type = "re", 
...
filename <- "HR_FinalModel_RE_N20.pdf"
plot_dir = "../plots/"
full_path <- file.path(plot_dir, filename)
